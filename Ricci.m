...
*                                                                         *
*                           By John M. Lee                                *
*            assisted by Dale Lear, John Roth, and Lee Nave               *
*                                                                         *
*                Copyright (c) 1992 - 1999 John M. Lee                    *
*                        All rights reserved                              *
*                                                                         *
...
Print[" -- Ricci Version 1.34 (January 2, 2000) --"];
Print["    Copyright 1992 - 2000 John M. Lee"];
Print["    Problem reports or suggestions to:"];
Print["      lee@math.washington.edu"];

If [ $VersionNumber >= 3.0,
     Print[" -- For formatted output in a Version 3 or 4 notebook:"];
     Print["    Cell menu"];
     Print["      -> Default Output Format Type"];
     Print["         -> OutputForm"]
    ];
...
Norm::usage = "Norm[x] is the norm of the tensor expression x. It
...
Variance::usage = "Variance -> Covariant or Contravariant is a
...
Covariant.
\nThe function Variance[x] returns a list of variances of
the tensor expression x, one for each index slot.";
...
SetAttributes[ { TensorQ,Tensor,TensorData,TensorMetricQ,rank }, HoldFirst];
...
Grad[(f_ /; Rank[f]===0) g_, opt___] := 
  f Grad[g,opt] + TensorProduct[g, Grad[f]];
...
HoldPattern[TensorTimesFormat[{c_},{s__},form___]] := 
  Infix[{Format[HoldForm[c],form],SymmetricProductFormat[{s},form]}," ",
  preclevel[150,400]];
HoldPattern[TensorTimesFormat[{c_,d__},{s___},form___]] := 
  Infix[{Format[HoldForm[c],form],TensorTimesFormat[{d},{s},form]}," ",
  preclevel[150,400]];
...
HoldPattern[SymmetricProductFormat[{x_,y__},form___]] := 
  Infix[{Format[HoldForm[x],form],
         SymmetricProductFormat[{y},form]}, " * ", preclevel[152,402]];
...
SetAttributes[{Tensor,TensorQ,TotalRank,TensorData}, HoldFirst];
...
defSymmetryRelations[Tensor[t_,{},{}], TensorSymmetry[_,_,symlist_] ] :=
  Module[{i,len,perm,sgn,order,count,p},
    ...
    For[i = 1, i < len, i++,
        perm = symlist[[i]];
        sgn  = symlist[[++i]];
        For[order = PermutationOrder[perm]; count = 1; p = perm,
            count < order,
            count++; p = p[[perm]],
...
PermutationOrder[p_?PermutationQ] :=
...
TCompute[ f_?mathFunctionQ[g_], {} ] := f[ TCompute[g,{}] ];
